...
function Projects() {
    ...
    useEffect(() => {
        fetch("https://api.github.com/users/Omar-Saad/repos?sort=created&page=1")
            .then((res) => res.json())
            .then((data) => {
                setProjects(data);
                // console.log(data);
            ...
            <div className="col projects-grid">
                {projects.map((project) => (
                    ...
                    <card key={project.id} className="card">
                        <a href={""+project.html_url} target="_blank" rel="noreferrer" className="card-link2">
                        <div className="card-body">
                            <h5 className="card-title">{project.name}</h5>
                            <h6 className="card-subtitle mb-2 text-muted">{project.description}</h6>
                            <p className="card-subtitle mb-2 text-muted">{project.language}</p>
